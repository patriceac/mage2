...
param(
  ...
  [ValidateSet('ambient', 'audio')] [string] $Scenario = 'ambient',
...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'verification\audio-checks.mjs') -Destination $ambientSupport
  if ($Mode -eq 'editor') {
    $ambientFixture = Join-Path $ambientRepo "output\$Scenario-fixture"
  ...
  & $BrokerScript -ArtifactPath $ambientPackage -ExecutableRelativePath '.ambient-verification\node.exe' `
    -Arguments ('"{PAYLOAD}\.ambient-verification\guest.mjs" "{OUTDIR}" "{PAYLOAD}" "' + $Mode + '" "' + $Scenario + '"') `
  ...
  if (Test-Path -LiteralPath $ambientSupport) {
    $resolvedSupport = (Resolve-Path -LiteralPath $ambientSupport).Path
    if ($resolvedSupport -ne $ambientSupport -or -not $resolvedSupport.StartsWith($ambientPackage + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) { throw 'Cleanup path escaped the canonical package.' }
    Remove-Item -LiteralPath $resolvedSupport -Recurse
  }
}
