...
import { configureAudioFixture } from "./verification/audio-fixture.mjs";

const root = path.resolve(import.meta.dirname, "..");
const audioScenario = process.argv.includes("--audio");
const destination = path.join(root, "output", audioScenario ? "audio-fixture" : "ambient-fixture");
...
if (audioScenario) configureAudioFixture(project, destination, ffmpeg);
...
for (const asset of content.assets) for (const variant of Object.values(asset.variants)) variant.sourcePath = `media/${path.basename(variant.sourcePath)}`;
