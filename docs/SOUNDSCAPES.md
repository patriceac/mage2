...
Dialogue videos fill the scene picture beneath the existing subtitle/choices panel in the player and Playtest. They show no asset name or native controls; Continue/choices advance explicitly, and ending playback does not advance the dialogue. A decoded video frame replaces the speaker portrait until the video is dismissed or fails. Menu pause and voice volume remain shared with other dialogue media. Audio/image dialogue and interaction-media presentation retain their existing behavior. Set `AUDIO_FIXTURE_DIALOGUE_VIDEO` to an optional reference video when generating the audio fixture to exercise this presentation with actual performance footage; generated derivatives remain in ignored output directories.
...
## Cinematic dialogue acceptance (2026-09-22)

The presentation follow-up passed five focused portrait/mixer tests, typechecking and release builds. The fixture used supplied performance footage rendered as a 10-second 960x540 H.264/AAC derivative. Chrome passed nine checks, including a verified 430px viewport; desktop and mobile screenshots show readable dialogue/choices, scene-sized video, and no duplicate portrait or media card. Continue, same-asset playback on the next node, menu pause, end without automatic advance, and portrait restoration were exercised.

Editor request `executable-test-20260922T144116936Z-13b71a5f` passed 12 checks; runtime request `executable-test-20260922T144449094Z-0d0cf069` passed nine. Final broker/guest/application verdicts and process cleanup passed, VMs were Off with networking disconnected, and child paths were deleted and absent. Both sets of screenshots were inspected. The runtime's optional log was locked; direct attachment inventory remains unavailable under the host policy, with empty broker payload/processing directories verified. The earlier release resources were preserved in the separate `f70b` worktree.
