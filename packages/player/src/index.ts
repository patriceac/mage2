...
export function createPlayerController(
  ...
  function selectHotspot(hotspotId: string, timeMs: number, sceneTimelineDurationMs?: number): HotspotResolution {
    return runPlayerAction((context) => {
      ...
      const resolution = applyEffects(hotspot.effects, context);
      if (context.halted) {
        return resolution;
      }

      if (hotspot.dialogueTreeId && !resolution.startedDialogueTreeId) {
        startDialogueWithinAction(hotspot.dialogueTreeId, context);
      ...
      if (hotspot.targetSceneId && !resolution.transitionedToSceneId) {
        requestSceneTransition(hotspot.targetSceneId, context);
