...
export function ScenesPanel({
  ...
  useEffect(() => {
    ...
    async function loadSceneAudioUrl() {
      if (!currentSceneAudioAsset) {
        setSceneAudioUrl(undefined);
        return;
      }

      const sourcePath = currentSceneAudioVariant?.proxyPath ?? currentSceneAudioVariant?.sourcePath;
      if (!sourcePath) {
        setSceneAudioUrl(undefined);
        return;
      }
  ...
  }, [currentSceneAudioAsset?.id, currentSceneAudioVariant?.proxyPath, currentSceneAudioVariant?.sourcePath]);
  ...
  useEffect(() => {
    ...
    const syncFromAudioClock = () => {
      updatePlayheadFromSceneAudio(
        getSceneAudioPlayheadMs(
          audio.currentTime,
          audio.duration,
          currentScene.sceneAudioDelayMs,
          currentSceneAudioVariant?.durationMs
        )
      );
    };
    ...
    const startDelayClock = (startingPlayheadMs: number) => {
      ...
      const delayMs = Math.max(currentScene.sceneAudioDelayMs, 0);
    ...
    const syncSceneAudioToPlayhead = (nextPlayheadMs: number) => {
      if (!sceneAudioUrl || currentAsset?.kind !== "image" || !currentScene.sceneAudioAssetId) {
      ...
      const syncState = resolveSceneAudioSyncState(
        nextPlayheadMs,
        currentScene.sceneAudioDelayMs,
        resolvePlayableDurationMs(audio.duration, currentSceneAudioVariant?.durationMs),
        currentScene.sceneAudioLoop
      );
      ...
      if (syncState.phase === "waiting") {
        ...
        if (playbackDirective.shouldScheduleDelayedPlayback) {
          ...
          sceneAudioTimeoutRef.current = window.setTimeout(() => {
            sceneAudioTimeoutRef.current = undefined;
            updatePlayheadFromSceneAudio(Math.max(currentScene.sceneAudioDelayMs, 0));
            void audio
              .play()
              .then(() => {
    ...
    const handleEnded = () => {
      cancelAnimationFrameSync();
      if (currentScene.sceneAudioLoop) {
        syncSceneAudioToPlayhead(0);
        return;
      }

      const durationMs = resolvePlayableDurationMs(audio.duration, currentSceneAudioVariant?.durationMs);
      sceneAudioPhaseRef.current = "ended";
      if (durationMs !== undefined) {
        updatePlayheadFromSceneAudio(Math.max(currentScene.sceneAudioDelayMs, 0) + durationMs);
        return;
      }
  ...
  useEffect(() => {
    ...
    if (!syncSceneAudioToPlayhead || !sceneAudioUrl || currentAsset?.kind !== "image" || !currentScene.sceneAudioAssetId) {
      return;
    }
